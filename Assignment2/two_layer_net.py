...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    hidden = torch.max(torch.zeros_like(hidden_before_ReLU),hidden_before_ReLU)
    scores = hidden @ W2 + b2
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_forward_backward(
    ...
    corret_prob = prob[torch.arange(N), y]

    loss = -torch.log(corret_prob).mean()
    ...
    db2 = dS
    grads["b2"] = db2.sum(dim=0)
    # print(h1.shape) #(N, H)

    dW2 = h1.t() @ dS # (H, N) X (N, C) -> (H, C)
    grads["W2"] = dW2  + 2 * reg * W2
    ########################  The Second Layer  ################################
    dReLU = dS @ W2.t() # (N, C) X (C, H) -> (N, H)
    hidden_before_ReLU = X @ W1 + b1
    dReLU[(hidden_before_ReLU < 0)] = 0

    db1 = dReLU
    grads["b1"] = db1.sum(dim=0)
...
def nn_predict(
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    scores = loss_func(params, X) # If the arg y is none, it will return scores! 
    #Check the loss_func
