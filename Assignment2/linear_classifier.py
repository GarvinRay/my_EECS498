...
def svm_loss_vectorized(
    ...
    #############################################################################
    # Replace "pass" statement with your code
    score = torch.mm(W.t(),X.t())
    num_train = X.shape[0]
    margin = score - score[y,torch.arange(num_train)] + 1.0
    margin[y,torch.arange(num_train)] = 0
    loss = torch.max(torch.zeros_like(margin),margin).sum() / num_train
    loss += reg * torch.sum(W * W)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    ...
    k = torch.sum(mask, dim = 0)
    mask[y, torch.arange(num_train)] = -k
    dW = torch.mm(mask, X).t() / num_train
    dW += 2 * reg * W
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def predict_linear_classifier(W: torch.Tensor, X: torch.Tensor):
    ...
    """
    y_pred = torch.zeros(X.shape[0], dtype=torch.int64) # (N,)
    ###########################################################################
    # TODO:                                                                   #
...
def svm_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [2e-3,5e-3,6e-3]
    regularization_strengths = [2e-3,1e-3,5e-3]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def test_one_param_set(
    ...
    train_acc = 100 * (y_train == y_train_pred).double().mean().item()
    val_acc = 100 * (y_pred == y_val ).double().mean().item()
    ############################################################################
    #                            END OF YOUR CODE                              #
    ############################################################################
...
def softmax_loss_naive(
    ...
    #############################################################################
    # Replace "pass" statement with your code
    # socre = torch.mm(W.t(),X.t()) # (C, D) x (D, N) -> (C, N)
    N = X.shape[0]
    C = W.shape[1]
    W1 = W.t() # (C, D)
    X1 = X.t() # (D, N)
    for i in range(N):
        score = torch.mv(W1, X1[:,i]) # (C,D) X (D,1) = (C,1)
        score = score - score.max()
        exp_score = torch.exp(score)
        p = exp_score / exp_score.sum() 
        corret_p = p[y[i]]
        
        loss += - torch.log(corret_p)

        dW += torch.outer(X1[:, i], p)  # (D, 1) X (1, C) -> (D, C)
        dW[:, y[i]] -= X1[:, i] 
    loss /= N
    loss += reg * torch.sum(W * W)
    dW = dW / N + 2 * reg * W
...
def softmax_loss_vectorized(
    ...
    #############################################################################
    # Replace "pass" statement with your code
    score = torch.mm(W.t(), X.t()) # (C, N)
    # print(score.shape)
    score -= torch.max(score, dim = 0, keepdim = True).values
    exp_score = torch.exp(score)
    # print(exp_score.shape)
    p = exp_score / exp_score.sum(dim = 0,keepdim=True) #(C, N) / (N,) -> (C, N)
    corret_p = p[y,torch.arange(X.shape[0])]
    # print(corret_p.shape)
    loss = -torch.log(corret_p).mean()
    loss += reg * torch.sum(W * W)
    
    mask = torch.zeros_like(p) # C , N
    mask[y, torch.arange(X.shape[0])] = 1 # C  ,N
    dW += torch.mm((p - mask), X).t() / X.shape[0]  # (C, N) X (N, D) -> (C, D) -> t()
    dW += 2 * reg * W
    #############################################################################
    #                          END OF YOUR CODE                                 #
    #############################################################################
...
def softmax_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [9e-2,1.5e-2,3e-2]
    regularization_strengths = [5e-3,1e-2]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
