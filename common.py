...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def __init__(self, out_channels: int):
        ...
        self.fpn_params.update({
          'c3_lateral': nn.Conv2d(in_channels=dummy_out['c3'].shape[1],out_channels= self.out_channels, kernel_size=(1, 1)),
    ...
    def forward(self, images: torch.Tensor):
        ...
        p5_upsample = F.interpolate(input=p5,scale_factor=2.0,mode="bilinear") # Call the Upsample API without implement it.
...
def get_fpn_location_coords(
    ...
    for level_name, feat_shape in shape_per_fpn_level.items():
        ...
        # Using the broadcast 
        # xc = level_stride * (torch.arange(W,device=device,dtype=dtype).reshape(-1,1) + 0.5) # (W, 1)
        # yc = level_stride * (torch.arange(H,device=device,dtype=dtype).reshape(1,-1) + 0.5) # (1, H)
        # # Get whole point in this feature map.

        # locations = torch.zeros(H, W, 2, dtype=xc.dtype, device=xc.device)
        # locations[:, :, 0] = xc
        # locations[:, :, 1] = yc
        # location_coords[level_name] = locations.reshape(-1, 2)
        ...
        # return tensor([[1, 4],
                ...
                #        [3, 4],
                #        [3, 5]])
        xc = (torch.arange(H, dtype=dtype, device=device) + 0.5) * level_stride
        yc = (torch.arange(W, dtype=dtype, device=device) + 0.5) * level_stride
        location_coords[level_name] = torch.cartesian_prod(xc, yc)
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
