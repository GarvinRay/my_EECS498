...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # train_idx = 0
    # for test in x_test:
    #     test_flattened = test.reshape(-1)
    #     for train in x_train:
    #         train_flattened = train.reshape(-1)
    #         dists[train_idx,test_idx] = torch.sum((train_flattened - test_flattened) ** 2)
    #         train_idx += 1
    #     train_idx = 0
    #     test_idx += 1
    for i, train in enumerate(x_train):
        ...
        for j, test in enumerate(x_test):
            test_flattened = test.reshape(-1)
            dists[i, j] = torch.sum((train_flattened - test_flattened) ** 2)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # idx_train = torch.arange(x_train.shape[0])
    # idx_test = torch.arange(x_test.shape[0])
    # Make the train and test into a rank-2 tensor.
    # The train will broadcast.
    x_train_flattened = x_train.reshape(num_train,-1)
    x_test_flattened = x_test.reshape(num_test,-1)
    for i in range(num_train):
        dists[i] = torch.sum((x_train_flattened[i] - x_test_flattened) ** 2,dim = 1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    train_flattened = x_train.reshape(num_train,-1)
    test_flattened = x_test.reshape(num_test,-1)

    train2 = train_flattened.pow(2).sum(dim = 1,keepdim=True)
    test2 = test_flattened.pow(2).sum(dim = 1,keepdim=True)
    test_mul_train = torch.mm(train_flattened,test_flattened.t())
    # The broadcast is wonderful!

    dists = train2 + test2.t() - 2 * test_mul_train
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    v,idx = torch.topk(dists, k, dim = 0, largest=False)
    for i in range(num_test):
        label = y_train[idx[:,i]]
        label, counts = torch.unique(label, sorted=True, return_counts=True)
        y_pred[i] = label[torch.argmax(counts)]
    # The unique(return_counts = True) is always used to vote.
    # By using it, we can get the  label with it emerge times.
    # The argmax() can return the max item' s idx.
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
            x_train_ = torch.cat(x_train_folds[:i] + x_train_folds[i+1:], dim=0)
            y_train_ = torch.cat(y_train_folds[:i] + y_train_folds[i+1:], dim=0)
            x_val = x_train_folds[i]
            y_val = y_train_folds[i]

            classifier = KnnClassifier(x_train_,y_train_)
            acc.append(classifier.check_accuracy(x_val,y_val,k,quiet=True))
