...
def create_tensor_of_pi(M: int, N: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x = torch.full((M, N), 3.14, dtype=torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def slice_assignment_practice(x: Tensor) -> Tensor:
    ...
    # x[2:4, :1] = torch.full((2,1),3)
    # x[2:4,1:2] = torch.full((2,1),4)
    # x[2:4,2:3] = torch.full((2,1),3)
    # x[2:4,3:4] = torch.full((2,1),4)
...
def shuffle_cols(x: Tensor) -> Tensor:
    ...
    y = x[ : ,col_idx]
    # There are some problems that how to make a copy of the first column??? But it seems that it works.
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def take_one_elem_per_col(x: Tensor) -> Tensor:
    ...
    col_idx = torch.arange(x.shape[1])#x.shape[0] is wrong.
    y = x[row_idx,col_idx]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    y = x.reshape(-1,3,4).transpose(0,1).reshape(3,8)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # x = torch.tensor(xs).reshape(len(xs[0]),len(xs))
    # y = x.sum(dim = 0) / ls
    # The tensor in xs has different length. How to solve?
