...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for out_c in stem_channels:
            conv = nn.Conv2d(in_c, out_c, kernel_size=3, stride=1, padding=1, bias=True)
        ...
        # Replace "pass" statement with your code
        self.pred_cls = nn.Conv2d(stem_channels[-1],num_classes,(3,3),stride=1,padding=1,bias=True)
        self.pred_box = nn.Conv2d(stem_channels[-1],4,(3,3),stride=1,padding=1,bias=True)
        self.pred_ctr = nn.Conv2d(stem_channels[-1],1,(3,3),stride=1,padding=1,bias=True)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        # Replace "pass" statement with your code
        for level,fpn_feat in feats_per_fpn_level.items():
            feat_cls = self.stem_cls(fpn_feat)
            feat_box = self.stem_box(fpn_feat) # The box and ctr sharing the same stem.

            logits_cls = self.pred_cls(feat_cls)
            logits_ctr = self.pred_ctr(feat_box)
            deltas_box = self.pred_box(feat_box)

            class_logits[level] = logits_cls.flatten(start_dim=2,end_dim=3).permute(0, 2, 1)
            boxreg_deltas[level] = deltas_box.flatten(start_dim=2,end_dim=3).permute(0, 2, 1)
            centerness_logits[level] = logits_ctr.flatten(start_dim=2,end_dim=3).permute(0, 2, 1)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def fcos_get_deltas_from_locations(
    ...
    # Replace "pass" statement with your code
    N = locations.shape[0]
    deltas = torch.full((N, 4), -1.0, dtype=locations.dtype, device=locations.device)
    ...
    deltas[is_valid, 0] = l[is_valid] / stride
    deltas[is_valid, 1] = t[is_valid] / stride
    deltas[is_valid, 2] = r[is_valid] / stride
    deltas[is_valid, 3] = b[is_valid] / stride
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def fcos_apply_deltas_to_locations(
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    valid_deltas = deltas.clip(min=0)
    xc,yc = locations.unbind(dim=1)
    l,t,r,b = (valid_deltas * stride).unbind(dim=1)
    x1 = xc - l
    y1 = yc - t
    x2 = xc + r
    y2 = yc + b
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    mask = (deltas < 0).any(dim=1)
    centerness = ((torch.min(l,r) * torch.min(t,b)) / (torch.max(l,r) * torch.max(t,b))).sqrt()
    centerness[mask] = -1
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
