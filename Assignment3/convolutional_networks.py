...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        x_pad = torch.nn.functional.pad(x, pad_dims, "constant", 0.)
            ...
            for f in range(F):
                for i in range(H_out):
                    for j in range(W_out):
                        out[n,f,i,j] = (x_pad[n,:,i*stride:i*stride+HH,j*stride:j*stride+WW] * w[f,:,:,:]).sum()
            out[n] += b[:, None, None]
        #####################################################################
        #                          END OF YOUR CODE                         #
        #####################################################################
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        for n in range(N):
            for i in range(H_out):
                for j in range(W_out):
                    pool_x = x[n,:,i*stride:i*stride+pool_height,j*stride:j*stride+pool_width].reshape(C,-1)
                    out[n,:,i,j],_ = torch.max(pool_x,dim=1)
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
            ...
            for c in range(C):
                for i in range(H_out):
                    for j in range(W_out):
                        ...
                        max_indice = torch.argmax(x_pool)
                        row = max_indice // pool_height
                        column = max_indice % pool_width
                        dx_pool = dx[n, c, i*stride:i*stride+pool_height, j*stride:j*stride+pool_width]
                        dx_pool[row, column] += dout[n,c,i,j] 
                        # The op "+=" is right, but the test code "stride >= pool_h/w"
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        # Replace "pass" statement with your code
        C, H, W = input_dims
        self.params["W1"] = torch.normal(mean=0,std=weight_scale,
                                        size=(num_filters,C,filter_size,filter_size),
                                        dtype=self.dtype,device=device) # W1:(F, C, HH, WW) 
        self.params["b1"] = torch.zeros(num_filters,
                                        dtype=dtype,
                                        device=device)
        H_out = H // 2
        W_out = W // 2
        input_flatten = H_out * W_out * num_filters
        self.params["W2"] = torch.normal(mean=0,
                                         std=weight_scale,
                                         size=(input_flatten,hidden_dim),
                                         dtype=dtype,
                                         device=device)
        self.params["W3"] = torch.normal(mean=0,
                                         std=weight_scale,
    ...
    def loss(self, X, y=None):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        out1,cache1 = Conv_ReLU_Pool.forward(x=X,w=W1,b=b1,conv_param=conv_param,pool_param=pool_param)
        out2,cache2 = Linear_ReLU.forward(x=out1,w=W2,b=b2)
        scores,cache3 = Linear.forward(out2,w=W3,b=b3)
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        ####################################################################
        # Replace "pass" statement with your code
        loss,ds = softmax_loss(x=scores,y=y)
        loss += (torch.sum(W1**2) + torch.sum(W2**2) + torch.sum(W3**2)) * self.reg 
        dout3,dW3,db3 = Linear.backward(dout=ds,cache=cache3)
        dout2,dW2,db2 = Linear_ReLU.backward(dout=dout3,cache=cache2)
        dout1,dW1,db1 = Conv_ReLU_Pool.backward(dout=dout2,cache=cache1)
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        filter = C
        for i in range(self.num_layers - 1): # 0 -> len(num_filters)
          if not weight_scale == "kaiming": # Initialize without using kaiming.
            self.params[f"W{i+1}"] = torch.normal(mean=0,std=weight_scale,size=(num_filters[i],filter,3,3),
                                        dtype=dtype,device=device)#In the VGG-Net, the kenerl is 3 X 3.
          else:
            self.params[f"W{i+1}"] = kaiming_initializer(filter,num_filters[i],K=3,relu=True,device=device,dtype=dtype)

          filter = num_filters[i]
          self.params[f"b{i+1}"] = torch.zeros(num_filters[i],dtype=dtype,device=device)
          if i in self.max_pools:
            current_H //= 2
            current_W //= 2
          if self.batchnorm:
            self.params[f'gamma{i+1}'] = torch.ones(filter, dtype=dtype, device=device)
            self.params[f'beta{i+1}'] = torch.zeros(filter, dtype=dtype, device=device)
            
        if not weight_scale == "kaiming":
          self.params[f"W{self.num_layers}"] = torch.normal(mean=0,std=weight_scale,size=(filter * current_H * current_W,num_classes),
                                              dtype=dtype,device=device)
          self.params[f"b{self.num_layers}"] = torch.zeros(num_classes,dtype=dtype,device=device)
        else:
          self.params[f"W{self.num_layers}"] = kaiming_initializer(filter * current_H * current_W,num_classes,K=None,relu=True,dtype=dtype,device=device)
          self.params[f"b{self.num_layers}"] = torch.zeros(num_classes,dtype=dtype,device=device)
        ################################################################
        #                      END OF YOUR CODE                        #
        ################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
          if not self.batchnorm:
            if i in self.max_pools:
                out[f"out{i+1}"],cache[f"cache{i+1}"] = Conv_ReLU_Pool.forward(x=out[f"out{i}"],
                                                w=self.params[f"W{i+1}"],b=self.params[f"b{i+1}"],
                                                conv_param=conv_param,pool_param=pool_param)
            else:
                out[f"out{i+1}"],cache[f"cache{i+1}"] = Conv_ReLU.forward(x=out[f"out{i}"],
                                                w=self.params[f"W{i+1}"],b=self.params[f"b{i+1}"],
                                                conv_param=conv_param)
          else:
            if i in self.max_pools:
              out[f"out{i+1}"],cache[f"cache{i+1}"] = Conv_BatchNorm_ReLU_Pool.forward(x=out[f"out{i}"],
                                              w=self.params[f"W{i+1}"],b=self.params[f"b{i+1}"],
                                              gamma=self.params[f"gamma{i+1}"],beta=self.params[f"beta{i+1}"],
                                              conv_param=conv_param,bn_param=self.bn_params[i],pool_param=pool_param)
            else:
              out[f"out{i+1}"],cache[f"cache{i+1}"] = Conv_BatchNorm_ReLU.forward(x=out[f"out{i}"],
                                              w=self.params[f"W{i+1}"],b=self.params[f"b{i+1}"],
                                              gamma=self.params[f"gamma{i+1}"],beta=self.params[f"beta{i+1}"],
                                              conv_param=conv_param,bn_param=self.bn_params[i])
        conv_n = out[f"out{self.num_layers-1}"]
        out[f"out{self.num_layers}"],cache[f"cache{self.num_layers}"] = Linear.forward(x=conv_n,
                                                w=self.params[f"W{self.num_layers}"],
                                                b=self.params[f"b{self.num_layers}"],)
        scores = out[f"out{self.num_layers}"]
        #####################################################
        #                 END OF YOUR CODE                  #
        #####################################################
        ...
        # Replace "pass" statement with your code
        dout = {}
        loss,ds = softmax_loss(scores,y=y)
        loss += self.reg * torch.sum(self.params[f"W{self.num_layers}"] ** 2)
        dout[f"dout{self.num_layers-1}"],dW_,grads[f"b{self.num_layers}"] = Linear.backward(ds,cache=cache[f"cache{self.num_layers}"])
        grads[f"W{self.num_layers}"] = dW_ + 2 * self.reg * self.params[f"W{self.num_layers}"]
        for i in reversed(range(self.num_layers-1)):
            loss += self.reg * torch.sum(self.params[f"W{i+1}"] ** 2) 
            if not self.batchnorm:
                if i in self.max_pools:
                    dout[f"dout{i}"],dW,grads[f"b{i+1}"] = Conv_ReLU_Pool.backward(dout[f"dout{i+1}"],cache[f"cache{i+1}"])
                    grads[f"W{i+1}"] = dW + 2 * self.reg * self.params[f"W{i+1}"]
                else:
                    dout[f"dout{i}"],dW,grads[f"b{i+1}"] = Conv_ReLU.backward(dout[f"dout{i+1}"],cache[f"cache{i+1}"])
                    grads[f"W{i+1}"] = dW + 2 * self.reg * self.params[f"W{i+1}"]
            else:
                if i in self.max_pools:
                    dout[f"dout{i}"],dW,grads[f"b{i+1}"],grads[f"gamma{i+1}"],grads[f"beta{i+1}"] = Conv_BatchNorm_ReLU_Pool.backward(dout[f"dout{i+1}"],cache[f"cache{i+1}"])
                    grads[f"W{i+1}"] = dW + 2 * self.reg * self.params[f"W{i+1}"]
                else:
                    dout[f"dout{i}"],dW,grads[f"b{i+1}"],grads[f"gamma{i+1}"],grads[f"beta{i+1}"] = Conv_BatchNorm_ReLU.backward(dout[f"dout{i+1}"],cache[f"cache{i+1}"])
                    grads[f"W{i+1}"] = dW + 2 * self.reg * self.params[f"W{i+1}"]
        #############################################################
        #                       END OF YOUR CODE                    #
        #############################################################
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    #########################################################
    # Replace "pass" statement with your code
    N, C, H, W = data_dict["X_train"].shape
    model = DeepConvNet(input_dims=(C, H, W), num_filters=[16, 32, 32],
                        max_pools=[0, 2], num_classes=10, 
                        weight_scale='kaiming', reg=2e-3,
    ...
    solver = Solver(model=model,data=data_dict, 
                    ...
                    print_every=300, verbose=True,
                    print_acc_every=1, device='cuda')# The adam api is useless? how to use it?
    #########################################################
    #                  END OF YOUR CODE                     #
    #########################################################
    return solver
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
    ...
    if K is None:
        ...
        ###################################################################
        # Replace "pass" statement with your code
        weight = torch.normal(mean=0,std=math.sqrt(gain / Din),size=(Din, Dout), dtype=dtype, device=device)
        ###################################################################
        #                            END OF YOUR CODE                     #
        ###################################################################
    else:
        ...
        ###################################################################
        # Replace "pass" statement with your code
        weight = torch.normal(mean=0,std=math.sqrt(gain / (Din*K*K)),size=(Dout, Din, K, K), dtype=dtype, device=device)
        # Shift the Din and Dout for the Conv
        ###################################################################
        #                         END OF YOUR CODE                        #
        ###################################################################
    return weight
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            x_hat = (x - mean) / torch.sqrt(var + eps) # (N, D)
            out = x_hat * gamma + beta # (D,)
    ...
    def backward(dout, cache):
        ...
        dmean = (-dx_hat / torch.sqrt(var + eps)).sum(dim=0) + dvar * torch.sum(-2.0 * (x - mean), dim=0) / N
    ...
    def backward_alt(dout, cache):
        ...
        dx = (gamma / (N * std)) * (N * dout - dout.sum(dim=0, keepdim=True) - x_hat * (dout * x_hat).sum(dim=0, keepdim=True))
        #################################################################
        #                        END OF YOUR CODE                       #
        #################################################################
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        # The permute() function is powerful!
