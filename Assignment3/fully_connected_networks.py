...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        x0 = x.reshape(x.shape[0], -1) # Important: We could not change it!!!
        out = x0 @ w + b
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        ###################################################################
        # Replace "pass" statement with your code
        self.params["W1"] = torch.normal(mean=0,
                                         std=weight_scale,
    ...
    def loss(self, X, y=None):
        ...
        b1 = self.params["b1"]
        b2 = self.params["b2"]
        out1,cache1= Linear_ReLU.forward(x=X,w=W1,b=b1)
        out2,cache2= Linear.forward(x=out1,w=W2,b=b2)
        scores = out2
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        dout2,dW2,db2 = Linear.backward(dout=ds,
                                        cache=cache2)
        dout1,dW1,db1 = Linear_ReLU.backward(dout=dout2,
                                             cache=cache1)
        grads["W2"] = 2 * self.reg * W2 + dW2
        grads["W1"] = 2 * self.reg * W1 + dW1
        grads["b2"] = db2
        grads["b1"] = db1
        ###################################################################
        #                     END OF YOUR CODE                            #
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        #######################################################################
        # Replace "pass" statement with your code
        self.all_dims = [input_dim] + hidden_dims + [num_classes]
        for i in range(self.num_layers):
          self.params[f"b{i+1}"] = torch.zeros(self.all_dims[i+1], 
                                              dtype=dtype, 
                                              device=device)
          self.params[f"W{i+1}"] = torch.normal(mean=0, std=weight_scale, 
                                                size=(self.all_dims[i], self.all_dims[i+1]), 
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
          Out[f"Out{i+1}"],Cache[f"Cache{i+1}"] = Linear_ReLU.forward(Out[f"Out{i}"],
                                                                      self.params[f"W{i+1}"], 
                                                                      self.params[f"b{i+1}"])
        scores,Cache[f"Cache{self.num_layers}"] = Linear.forward(Out[f"Out{self.num_layers - 1}"],
        ...
        dout = {}
        dout[f"dout{self.num_layers-1}"],dW_, grads[f"b{self.num_layers}"] = Linear.backward(ds,Cache[f"Cache{self.num_layers}"])
        # Set the start dout ---> Specifically compute the Linear Layer.
        ...
        for i in reversed(range(self.num_layers-1)): 
          ...
          dout[f"dout{i}"], dW, grads[f"b{i+1}"] = Linear_ReLU.backward(dout[f"dout{i+1}"],Cache[f"Cache{i+1}"]) 
          grads[f"W{i+1}"] = 2 * self.reg * self.params[f"W{i+1}"] + dW
        ###########################################################
        #                   END OF YOUR CODE                      #
        ###########################################################
...
def rmsprop(w, dw, config=None):
    ...
    next_w = - learning_rate * dw / (torch.sqrt(cache) + eps) + w
    config["cache"] = cache
    # The derivation process is in the book.
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def adam(w, dw, config=None):
    ...
    m_ = m / (1 - beta1 ** t)
    v_ = v / (1 - beta2 ** t)
    next_w = - learning_rate * m_ / (torch.sqrt(v_) + eps) + w
    config["m"] = m
    config["v"] = v
    #########################################################################
    #                              END OF YOUR CODE                         #
